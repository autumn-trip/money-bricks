...
import {updateFirstLoginFlag} from "../../member/api/memberApi";
...
const useCustomLogin = () => {
    ...
    const doLogin = async (loginParam) => {
        ...
        // 로그인 성공 후 첫 로그인 플래그 업데이트
        if (data.firstLoginFlag) {
            // 서버에서 첫 로그인 플래그 업데이트
            await updateFirstLoginFlag(); // 첫 로그인 처리 후 플래그 변경
        }

        // 로그인 후 만기 처리 체크
        try {
            await checkMaturityAndProcess(); // 만기 처리 API 호출
            console.log("만기 처리 완료");
        } catch (error) {
